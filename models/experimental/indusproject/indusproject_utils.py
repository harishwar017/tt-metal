...
def cache_weights_in_weka(device, dtype, reset_seeds):
    ...
    file_name = "/tt-metal/models/experimental/indusproject/weights/"
...
def store_weights(model_version, file_name, base_address, dtype):
    model_hf = AutoModelForCausalLM.from_pretrained("nickmalhotra/ProjectIndus")
    print(f"Storing weights for model version: {model_version} at path: {file_name}")
    state_dict = model_hf.state_dict()
    weights_dtype = dtype
    ...
    for key, value in state_dict.items():
        if base_address == "" and (
            ...
            # print("Storing weights in weka path...")
            torch.save(value, file_name + str(key) + ".pt")
            continue
        ...
        if (os.path.exists(file_name + str(key) + str(weights_dtype) + ".tensorbin")) or (
            ...
            # print("Weights already exist or not in base address, skipping...")
        ...
        print("Storing weights in dump path...")
        ttnn.dump_tensor(file_name + str(key) + str(weights_dtype) + ".tensorbin", value)
...
def get_tt_cache_path(model_version):
    ...
    # else:
    Path(f"models/experimental/indusproject/datasets/{model_version}").mkdir(parents=True, exist_ok=True)
    return str(Path(f"models/experimental/indusproject/datasets/{model_version}")) + "/"
