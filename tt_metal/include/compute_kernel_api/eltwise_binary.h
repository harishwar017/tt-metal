...
ALWI void binary_tiles_init(uint32_t icb0, uint32_t icb1, bool acc_to_dest = false) {
    MATH((
        llk_math_eltwise_binary_init_with_operands<eltwise_binary_type, NONE, static_cast<MathFidelity>(MATH_FIDELITY)>(
...
ALWI void binary_dest_reuse_tiles(uint32_t in_cb_id, uint32_t in_tile_index, uint32_t dst_tile_index) {
    ...
    MATH((llk_math_eltwise_binary<
          ...
          DST_ACCUM_MODE,
          static_cast<MathFidelity>(MATH_FIDELITY),
          binary_reuse_dest>(in_tile_index, in_tile_index, dst_tile_index, true)));
}
