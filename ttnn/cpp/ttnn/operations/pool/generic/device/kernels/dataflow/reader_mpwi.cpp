...
ALWI void initialize_return_indices_data() {
    ...
    if (start_row <= pad_t) {
        ...
        uint32_t global_top_left_pad_idx = -(uint32_t)pad_l - (uint32_t)pad_t * (uint32_t)in_w;
        ...
        uint32_t leftmost_valid_index = (start_row - (uint32_t)pad_t) * (uint32_t)in_w;
        uint32_t start_row_left_pad_idx = leftmost_valid_index - (uint32_t)pad_l;
        init_index = start_row_left_pad_idx + start_col;
    } else {
        // top left is in valid region, we choose the valid index
        init_index = (start_row - (uint32_t)pad_t) * (uint32_t)in_w + (start_col - (uint32_t)pad_l);
    }
    ...
    if constexpr (indexes_32_bit) {
        volatile tt_l1_ptr uint32_t* idx_ptr =
            reinterpret_cast<volatile tt_l1_ptr uint32_t*>(get_write_ptr(in_idx_cb_id));
        for (uint32_t h = 0; h < kernel_h; ++h) {
            for (uint32_t w = 0; w < kernel_w; ++w) {
                uint32_t hw = h * kernel_w + w;
...
ALWI void read_kernel_with_top_left_index(uint32_t ind, uint32_t in_l1_read_base_addr) {
    ...
    uint32_t max_write_inc = TILE_WIDTH * BYTES_PER_ELEM;
...
void kernel_main() {
    ...
    // MPWI always returns indices - this compile-time arg exists but is always true for this kernel
    ...
    // MPWI always needs to initialize the in_cb and uses the clear value CB to initialize DST for large kernels
